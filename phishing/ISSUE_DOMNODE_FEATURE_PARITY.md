...
- Quick diagnostic (no retrain): keep `domNodeCount` logged and use it as a monitoring/alert signal. Optionally, add a rule-based override for certain combinations (domNodeCount + suspiciousScriptCount) if they point to false-positive.
